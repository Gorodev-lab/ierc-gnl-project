...
Capas incluidas en ierc_golfo_california.gpkg (CRS: EPSG:4326 - WGS 84):
  1. `proyectos_gnl` (Puntos/Polígonos): 11 proyectos GNL consolidados en el Golfo y Noroeste.
  2. `gasoductos_infraestructura_gnl` (Líneas): Trazados conocidos y proyectados de ductos GNL.
...
OUTPUT_DIR = BASE_DIR / 'causanaturadata' / 'output'
...
if gnl_geojson.exists():
    ...
    # Fallback if json not processed
    ...
    proj_features = [{
        ...
        'latitud': p['latitud'],
        'longitud': p['longitud'],
        'geometry': Point(p['longitud'], p['latitud'])
...
pipelines = [
    ...
    {
        'ducto_id': 'DUC_CORREDOR_NORTE',
        'nombre': 'Gasoducto Corredor Norte Sinaloa',
        'operador': 'Gasoducto Corredor Norte',
        'estatus': 'En evaluación',
        'longitud_km': 320.0,
        'geometry': LineString([(-108.25, 25.40), (-108.01, 24.89)])
    }
]
...
for lat, lon in focus_coords_res8:
    for cell in h3.grid_disk(h3.latlng_to_cell(lat, lon, 8), 18):
