...
class SpatialValidator:
    ...
    def validate_bounding_box(self, geometry: Union[Point, Polygon, MultiPolygon]) -> bool:
        ...
        try:
            if isinstance(geometry, Point):
                ...
                if not in_bbox:
                    logger.warning(f"  Punto fuera de bbox: ({lat}, {lon})")
                return in_bbox
                ...
                if not in_bbox:
                    logger.warning(f"  Geometría parcialmente fuera de bbox: bbox=({min_lat}, {min_lon}, {max_lat}, {max_lon})")
    ...
    def reproject_geometry_no_deformation(self, geometry: Union[Point, Polygon, MultiPolygon]) -> str:
        ...
        try:
            ...
            if not self.validate_bounding_box(reprojected_geometry):
                logger.warning("  Geometría reproyectada fuera de bbox, ajustando...")
        ...
        try:
            ...
            with self.db_engine.connect() as conn:
                ...
                if len(df_fisheries) == 0:
                    ...
                    logger.warning(f"  Celda {h3_cell_id} sin datos en últimos {self.config.historical_data_years} años")
    ...
    def batch_validate_geometries(self, gdf: gpd.GeoDataFrame, source_name: str) -> gpd.GeoDataFrame:
        ...
        try:
            ...
            if invalid_count > 0:
                logger.warning(f"  {invalid_count} geometrías fuera de bbox en {source_name}")
...
if __name__ == "__main__":
    from ..utils.logging import setup_logging
    setup_logging("ierc_gnl.spatial_validator")
    print("Spatial Validator module loaded")