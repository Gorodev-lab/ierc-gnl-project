...
logger.warning(f"  Geometrías nulas detectadas en {config.name}")
...
if not in_bbox:
logger.warning(f"  Algunas coordenadas están fuera del Golfo de California")
...
logger.warning(f"  Error en conversión H3: {e}")
return None, "Invalid"
...
logger.debug(f"  Error en conversión H3: {e}")
...
if len(df) == 0:
logger.warning("  No se encontraron registros de metaneros o barcos pesqueros")
return pd.DataFrame()
...
logger.warning("  Geometrías inválidas detectadas, reparando...")
...
"""
if len(df) == 0:
logger.warning(f"  No hay datos para insertar en {config.target_table}")
return 0
