#!/usr/bin/env python3
"""
build_geopackage_v2.py
----------------------
Construye el repositorio GeoPackage v2.0 estandarizado OGC para Causa Natura Data (POA 2026).
Autores: Juan Carlos Barrera (JCB - Consultor Senior) & Enrique Gorosave (EG - Analista GIS)

Capas incluidas en ierc_golfo_california_v2.gpkg (CRS: EPSG:4326 - WGS 84):
  Gabinete (1-7):
    1. `proyectos_gnl` (Puntos): Ubicación de terminales GNL.
    2. `gasoductos_infraestructura_gnl` (Líneas): Trazados conocidos y proyectados.
    3. `localidades_estudio_ierc` (Puntos): Punta Chueca, Puerto Libertad, Guaymas.
    4. `anp_habitats_criticos` (Polígonos): ANP (CONANP) y manglares/pastos marinos.
    5. `zonas_pesqueras_pangas` (Polígonos): Sitios PANGAS históricos con `uid_espaciotemporal`.
    6. `riqueza_relativa_pesquera` (Polígonos): Capa de riqueza acumulada.
    7. `grilla_h3_riesgo` (Polígonos): Grilla H3 adaptativa (Res 8 / Res 9).
  Trabajo de Campo 2026 (8-12):
    8. `campo_rutas_pesqueras` (LineString): Rutas pesqueras quincenales digitalizadas.
    9. `campo_zonas_pesca_quincenales` (MultiPolygon): Caladeros primarios y secundarios.
    10. `campo_sitios_bioculturales_comcaac` (Point/Polygon): Sitios bioculturales/sagrados.
    11. `campo_puntos_desembarque_costo` (Point): Sitios de desembarque y costos por viaje.
    12. `campo_interaccion_fondeaderos_gnl` (Polygon): Zonas de fondeo y buques metaneros.
"""
...
DELIVERABLE_V2_DIR = BASE_DIR / 'deliverables' / 'v2_geopackage'
OUTPUT_GPKG_V2 = DELIVERABLE_V2_DIR / 'ierc_golfo_california_v2.gpkg'

DELIVERABLE_V2_DIR.mkdir(parents=True, exist_ok=True)

print("=======================================================================")
print("Generación del GeoPackage Entregable v2.0 (Causa Natura Data - JCB/EG)")
print(f"Archivo destino: {OUTPUT_GPKG_V2}")
print("=======================================================================")
...
gdf_proyectos.to_file(OUTPUT_GPKG_V2, layer='proyectos_gnl', driver='GPKG')
...
gdf_pipelines.to_file(OUTPUT_GPKG_V2, layer='gasoductos_infraestructura_gnl', driver='GPKG')
...
gdf_locs.to_file(OUTPUT_GPKG_V2, layer='localidades_estudio_ierc', driver='GPKG')
...
gdf_anp.to_file(OUTPUT_GPKG_V2, layer='anp_habitats_criticos', driver='GPKG')
...
gdf_pangas.to_file(OUTPUT_GPKG_V2, layer='zonas_pesqueras_pangas', driver='GPKG')
...
if riqueza_geojson_path.exists():
    ...
    gdf_riqueza.to_file(OUTPUT_GPKG_V2, layer='riqueza_relativa_pesquera', driver='GPKG')
    print(f"   Capa 'riqueza_relativa_pesquera' creada ({len(gdf_riqueza)} polígonos).")

# ── 7. Capa: grilla_h3_riesgo ────────────────────────────────────────────────
print("\n7/12 Construyendo capa 'grilla_h3_riesgo'...")
h3_cell_data = []
for lat, lon in [(31.0833, -114.8500), (29.0000, -113.5000), (30.5000, -114.0000)]:
    center_h3 = h3.latlng_to_cell(lat, lon, 8)
    for cell in h3.grid_disk(center_h3, 18):
        h3_cell_data.append((cell, 8))

for lat, lon in [(29.9107, -112.6835), (27.9179, -110.9039), (28.9886, -112.1603)]:
    center_h3 = h3.latlng_to_cell(lat, lon, 9)
    for cell in h3.grid_disk(center_h3, 15):
...
for cell, res in h3_cell_data:
    ...
    min_dist_km = min([math.sqrt((lat - p['latitud'])**2 + (lon - p['longitud'])**2) * 111.0 for p in projects_list])
    ...
    ierc_score = ((amenaza * 0.20) + (exposicion * 0.20) + (sensibilidad * 0.15) + (0.70 * 0.15) + (0.85 * 0.15) + (0.70 * 0.15)) * 100.0
    nivel = "Alto" if ierc_score >= 75.0 else ("Moderado" if ierc_score >= 50.0 else "Bajo")
...
gdf_h3.to_file(OUTPUT_GPKG_V2, layer='grilla_h3_riesgo', driver='GPKG')
print(f"   Capa 'grilla_h3_riesgo' creada ({len(gdf_h3)} celdas H3).")
...
gpd.GeoDataFrame(dummy_routes, crs="EPSG:4326").to_file(OUTPUT_GPKG_V2, layer='campo_rutas_pesqueras', driver='GPKG')
...
gpd.GeoDataFrame(dummy_zonas, crs="EPSG:4326").to_file(OUTPUT_GPKG_V2, layer='campo_zonas_pesca_quincenales', driver='GPKG')
...
gpd.GeoDataFrame(dummy_bio, crs="EPSG:4326").to_file(OUTPUT_GPKG_V2, layer='campo_sitios_bioculturales_comcaac', driver='GPKG')
...
gpd.GeoDataFrame(dummy_puntos, crs="EPSG:4326").to_file(OUTPUT_GPKG_V2, layer='campo_puntos_desembarque_costo', driver='GPKG')
...
gpd.GeoDataFrame(dummy_fondeo, crs="EPSG:4326").to_file(OUTPUT_GPKG_V2, layer='campo_interaccion_fondeaderos_gnl', driver='GPKG')

print(f"\n¡GeoPackage v2.0 generado exitosamente en:\n   {OUTPUT_GPKG_V2}!")
