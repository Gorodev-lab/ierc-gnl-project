...
| **Capas Dashboard** | 14+ capas vectoriales interactivas (Next.js 16) |
...
|------------|----------------|-------------|------------------|
| **Amenaza (H)** | 0.20 | Infraestructura GNL, ruido, rutas metaneros | ASEA MIA, GFW rutas |
| **Exposición (H)** | 0.20 | Esfuerzo pesquero VMS + pangas | GFW, PANGAS |
...
| **Capacidad Adaptativa (V)** | 0.15 | Gobernanza GAGE, diversificación, acceso a crédito | GAGE, encuestas |
...
### 5.2 Nuevas Capacidades v2.0 (2026-08-06)
...
### 5.4 Auditoría de Ejecuciones — `lakehouse/metadata/runs.jsonl`
...
### 5.5 Catálogo de Datos — `DataCatalog` (`src/data/catalog/catalog.py`)

- **Datasets:** `datasets.json` (metadatos completos: schema, lakehouse_paths, quality, CDC)
- **Ejecuciones:** `runs.jsonl` (lineage completo, append-only)
- **Derived versioning:** `register_derived_dataset()` embebe lineage en `schema.lineage`

```python
schema["lineage"] = {
    "sources": ["gfw_fishing_effort", "nasa_chlor_a", ...],
    "transform_version": "v2.1_20260806",
...
└── public/data/                 # 17 archivos GeoJSON estáticos
```
...
### 6.3 Capas Base Disponibles (14 Capas Vectoriales)
...
|------------|---------|---------|
| `RiskMap.tsx` | 36 KB | Mapa Leaflet H3 + navegación rápida 4 terminales GNL + 14 capas |
...
### 6.7 Navegación Rápida a Terminales GNL (Sidebar)
...
├── src/
│   ├── data/
│   │   ├── ingestion/          # Pipeline ingesta (9 ingesters + base + factory)
...
│   └── data_catalog.yaml       # Catálogo declarativo 12 Silver + 6 Gold
├── scripts/                    # 25+ scripts operacionales
├── tests/unit/                 # 45 tests (storage, catalog, h3, ierc, RAI, spatial, MC, E2E)
├── .github/workflows/ci.yml    # Pipeline CI/CD 5 jobs
└── README.md                   # Este archivo (actualizado v2.1)
```
...
### 10.5 CI/CD Pipeline (`.github/workflows/ci.yml`)
| Job | Descripción |
|-----|-------------|
| **test** | Unit tests Python (45 tests) |
| **lint** | Syntax check módulos core |
...
## 12. DOCUMENTACIÓN TÉCNICA VINCULADA

- **Inventario técnico v2.1:** `INVENTARIO_DATOS_IERC_GNL_v2.1.md`
- **Reporte detallado oficial:** `REPORTE_INVENTARIO_DETALLADO_IERC_GNL.md` (este archivo)
- **Metadata GeoPackage Meta 1:** `REPORTE_INVENTARIO_GEOPACKAGE.md`
- **Configuración Lakehouse:** `config/lakehouse.yaml`
- **Catálogo Declarativo:** `config/data_catalog.yaml`
- **Formulación Matemática:** `docs/metodologia/Nota_Metodologica_Ajustada_JCB_EG.md`
- **Matriz de Vacíos:** `docs/metodologia/Inventario_y_Matriz_Vacios_Geoespaciales_EG.md`
- **Pipeline CI/CD:** `.github/workflows/ci.yml`
- **Estilo Dashboard:** `STYLE_GUIDE.md` (Esoteria Design System v1.1)

---

*Reporte generado por Enrique Gorosave Meza, Analista de Datos y SIG, Causa Natura Center — 2026-08-06*